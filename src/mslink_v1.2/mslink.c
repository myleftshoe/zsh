...
void getParameters(int argc, char *argv[]) {
	...
	if (LNK_TARGET == NULL || OUTPUT_FILE == NULL || showHelp) {
		printf("# mslink v1.2 compilé le %s à %s\n"
		"# Cette application permet de créer un Raccourci Windows (Fichier .LNK) sans dépendances avec les librairies Microsoft.\n"
		"\n"
		"Usage :\n"
		"%s -l cible_du_fichier_lnk [-n description] [-w working_dir] [-a cmd_args] [-i icon_path] -o mon_fichier.lnk [-p]\n"
		"\n"
		"Options :\n"
